 ...
 * frameworks/base/include/picoEngine_AndroidWrapper/TtsEngine.h
 *
 */
public class SynthProxy {

    static {
        System.loadLibrary("picoEngine_androidWrapper");
    }

    private final static String TAG = "SynthProxy";

    // Default parameters of a filter to be applied when using the Pico androidEngineStruct.
     ...
     */
    public SynthProxy(String engineConfig) {
        boolean applyFilter = true; //false; //shouldApplyAudioFilter(nativeSoLib);
        //Log.v(TAG, "About to load "+ nativeSoLib + ", applyFilter=" + applyFilter);
        mJniData = nativeSetup(engineConfig);
        if (mJniData == 0) {
            throw new RuntimeException("Failed to load picoTTS");
        }
    ...
    // HACK: Apply audio filter if the androidEngineStruct is pico
     ...
     * Return codes are defined in android.speech.picoEngine_AndroidWrapper.TextToSpeech
     */
    ...
    /**
     * Updates the androidEngineStruct configuration.
     */
    ...
    /**
     * Returns the currently set language, country and locale information.
     */
    ...
    private native final long nativeSetup(String engineConfig);
