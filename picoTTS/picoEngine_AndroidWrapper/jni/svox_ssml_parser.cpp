...
//EJ : can't find this include anymore in cutils
#include "cutils_jstring.h"
...
extern const char * createPhonemeString( const char * xsampa, int length );
...
void SvoxSsmlParser::startElement(const XML_Char* element, const XML_Char** attributes)
{
    ...
    size_t xsampasize = 0;
      ...
      xsampasize = strlen(xsampastr);
        ...
        {
            const char* phonstr = createPhonemeString(xsampastr, strlen(xsampastr) + 1);
            free(xsampastr);
            xsampastr = NULL;
